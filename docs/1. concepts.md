...
### What the Centrality Image Actually Looks Like

We ran `pdg_to_image.py` on both functions and visualised the result.
The saved image is at `docs/figures/pdg_centrality_images.png`.

**What you see:**
```
Row 1 (VULNERABLE):  6 nodes, 9 edges
Row 2 (SAFE):       11 nodes, 17 edges

Columns: Degree centrality | Katz centrality | Closeness centrality
```

**Why vulnerable looks different:**

The vulnerable function has only 6 nodes. Two of them — `user_input` and `strcpy` — have many edges passing through them (high degree, high Katz). This creates **large, bright concentrated blobs** in the top-left corner of the image.

The safe function has 11 nodes. The bounds-check logic (`strncpy`, the null terminator, the size subtraction) adds intermediate nodes that split up the dependency load. The hot spots become **smaller, more spread out, dimmer**.

```
VULNERABLE image:               SAFE image:
  Large bright blob               Several smaller blobs
  (strcpy dominates)              (load distributed across
                                   bounds-check nodes)
```

This is the visual pattern the CNN learns to classify.
A function where one node dominates all paths → suspicious.
A function with distributed, balanced dependency load → likely safe.

**The Katz channel is the brightest** (middle column) because Katz counts all paths of all lengths to a node, amplifying the signal from highly-connected nodes even more than simple degree does.
