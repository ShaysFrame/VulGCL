...
txt(ax, 2.0, 7.08, "higher score = more critical to information flow",
...
ax.plot([brx, brx+0.12, brx+0.12, brx], [brt, brt, brb, brb],
        color=C_L, lw=1.4, zorder=8)
txt(ax, brx+0.2, (brt+brb)/2, "top-10\nselected", ha="left", va="center",
        fontsize=7.2, color=C_L, fontweight="bold")

# ── CENTER: arrow ─────────────────────────────────────────────────────────────
pts = np.array([[4.55,5.5],[5.15,5.5],[5.15,5.78],[5.7,5.25],
                [5.15,4.72],[5.15,5.0],[4.55,5.0]])
ax.fill(pts[:,0], pts[:,1], color=C_L, zorder=8, alpha=0.90)
txt(ax, 5.12, 5.25, "focus\nslice", ha="center", va="center",
...
for i, (line, vuln) in enumerate(code_slice):
    y = 6.18 - i*0.37
    if vuln:
        box(ax, 5.9, y-0.16, 3.95, 0.32, "#FFEBEE", "#FFCDD2", lw=0, rad=0.02, zo=4)
...
txt(ax, 11.9, 6.78, "BPE Tokenization", ha="center", va="center",
        fontsize=7.5, fontweight="bold", color=C_L)
tokens = ["[CLS]","int","buffer","[10]","if","x",">","0","buffer","[x]","=","0","[SEP]"]
txt(ax, 11.9, 6.3, "  ".join(tokens[:7]),
        ha="center", va="center", fontfamily="monospace", fontsize=6.2, color="#4A148C")
txt(ax, 11.9, 6.05, "  ".join(tokens[7:]) + "  ...",
        ha="center", va="center", fontfamily="monospace", fontsize=6.2, color="#4A148C")
...
txt(ax, 11.9, 2.9, "h_L",     ha="center", va="center", fontsize=11, fontweight="bold", color=C_L)
