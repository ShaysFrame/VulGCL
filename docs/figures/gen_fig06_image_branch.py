...
title(ax, 7.0, 7.78, "Image Branch — PDG centrality scores encoded as a visual signature", fs=12.5)
...
ch_data = [
    np.clip(rng.normal(0.5, 0.15, (10,10)) + np.linspace(0,0.4,10).reshape(-1,1), 0, 1),
    np.clip(rng.normal(0.4, 0.18, (10,10)) + np.linspace(0,0.3,10).reshape(1,-1), 0, 1),
...
cnn_layers = [
    (9.85, 3.5, 1.8, 2.6,  "Conv + BN\n+ ReLU",    "#C8E6C9", C_I),
    (10.1, 3.7, 1.8, 2.2,  "Conv + BN\n+ ReLU",    "#A5D6A7", C_I),
    (10.35,3.9, 1.8, 1.8,  "Conv + BN\n+ ReLU",    "#81C784", C_I),
]
...
txt(ax, 13.13, 5.22, "h_I",    ha="center", va="center", fontsize=11, fontweight="bold", color=C_I)
txt(ax, 13.13, 4.75, "∈  ℝ²⁵⁶", ha="center", va="center", fontsize=8.5, color=C_I)
